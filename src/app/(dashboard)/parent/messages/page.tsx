...
export default function ParentMessagesPage() {
    ...
    const teachers = useQuery(api.communications.getTeachersForParent);
    ...
    const handleStartChat = async (teacherId: Id<"users">) => {
        try {
            const convId = await startConversation({ otherUserId: teacherId });
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-zinc-500">Communicate with your child's teachers</p>
                </div>
                <Dialog open={newChatOpen} onOpenChange={setNewChatOpen}>
                    ...
                    <DialogContent>
                        ...
                        <div className="space-y-3 py-4">
                            {teachers?.length === 0 ? (
                                <p className="text-zinc-500 text-center py-4">No teachers available.</p>
                            ) : (
                                teachers?.map((teacher) => (
                                    <button
                                        key={teacher._id}
                                        onClick={() => handleStartChat(teacher._id)}
                                        className="w-full flex items-center gap-3 p-3 rounded-lg border border-zinc-200 hover:border-blue-300 hover:bg-blue-50/50 transition-all text-left"
                                    >
                        ...
                        <div className="divide-y divide-zinc-100 max-h-[500px] overflow-y-auto">
                            {conversations.length === 0 ? (
                                <div className="p-6 text-center text-zinc-500">
                                    ...
                                    <p className="text-sm">Start a chat with a teacher</p>
                                </div>
                            ) : (
                    ...
                    {!selectedConversation ? (
                        <div className="flex-1 flex items-center justify-center text-zinc-500">
                            <div className="text-center">
                                ...
                                <p className="text-sm">Or start a new one with a teacher</p>
                            </div>
                        </div>
                    ) : (
                        <>
                            ...
                            <CardHeader className="border-b border-zinc-100 pb-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <Badge variant="outline" className="text-xs">{selectedConv?.otherUser?.role}</Badge>
                                    </div>
                                </div>
                            </CardHeader>
                            ...
                            <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
                                {messages === undefined ? (
                                    <LoadingAnimation message="Loading messages..." />
                                ) : messages.length === 0 ? (
